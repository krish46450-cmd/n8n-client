...
# Initialize logging - production-safe (logs directory may not exist)
try:
    ...
    # Fallback to console logging if logs directory can't be created
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s %(levelname)s: %(message)s'
    )
...
email_notifier = safe_init(EmailNotifier, "EmailNotifier")
...
def submit_support():
    ...
    if form.validate_on_submit():
        try:
            ...
            if 'attachments' in request.files:
                ...
                for file in files:
                    if file and file.filename:
                        ...
                        upload_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'tickets')
                        ...
                        # Store relative path
                        attachment_paths.append(f"tickets/{unique_filename}")
                        logging.info(f"Saved attachment: {unique_filename}")
            ...
            if email_notifier:
                try:
                    email_notifier.send_ticket_created_notification(
                        current_user.email,
            ...
            if is_ajax:
                return jsonify({
                    'status': 'error',
                    'message': f'Error submitting support request: {str(e)}'
                }), 500
            else:
        ...
        if is_ajax:
            ...
            # Return HTML with flash messages for regular requests
            for field, errors in form.errors.items():
                for error in errors:
                    flash(f"{field}: {error}", "error")
                    logging.warning(f"Validation error - {field}: {error}")
...
@login_required
def chat():
    """Handle chat messages with Gemini AI (with optional image support)"""
    try:
        # Check if request has multipart/form-data (for image uploads)
        image_file = None
        image_data = None
        ...
        if request.content_type and 'multipart/form-data' in request.content_type:
            ...
            if 'image' in request.files:
                ...
                if image_file and image_file.filename:
                    ...
                    image_file.seek(0)  # Reset file pointer if needed later
        else:
...
try:
    ...
    logging.warning(f"Could not create some directories: {e}")
...
@login_required
def export_tickets():
    """Export user's support tickets to CSV"""
    try:
        import csv
        ...
        # Write header
        writer.writerow(['Ticket ID', 'Title', 'Description', 'Error Code', 'Priority', 'Status',
                        'Created At', 'Updated At', 'Resolved At', 'Solution'])
        ...
        output.headers["Content-Disposition"] = f"attachment; filename=support_tickets_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        output.headers["Content-type"] = "text/csv"

        logging.info(f"User {current_user.username} exported {len(tickets)} tickets")
        return output

    except Exception as e:
        logging.error(f"Error exporting tickets: {str(e)}")
        flash("Error exporting tickets. Please try again.", "error")
        return redirect(url_for('index'))


@app.route('/export/conversations')
@login_required
def export_conversations():
    """Export user's chatbot conversations to CSV"""
    try:
        import csv
        from io import StringIO
        from flask import make_response
        ...
        # Write header
        writer.writerow(['Conversation ID', 'Error Code', 'Status', 'Escalated', 'Created At',
                        'Message Role', 'Message Content', 'Message Time'])

        # Write data
        for conv in conversations:
            messages = ConversationMessage.query.filter_by(conversation_id=conv.id).order_by(ConversationMessage.created_at).all()
            for msg in messages:
                writer.writerow([
                    conv.conversation_id,
                    conv.error_code or '',
                    conv.status,
                    'Yes' if conv.escalated else 'No',
        ...
        # Create response
        output = make_response(si.getvalue())
        output.headers["Content-Disposition"] = f"attachment; filename=chatbot_conversations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        output.headers["Content-type"] = "text/csv"

        logging.info(f"User {current_user.username} exported {len(conversations)} conversations")
        return output

    except Exception as e:
        logging.error(f"Error exporting conversations: {str(e)}")
        flash("Error exporting conversations. Please try again.", "error")
        return redirect(url_for('index'))
...
if __name__ == '__main__':
    ...
    print(f"Flask server will start on: http://{host}:{port}")
    print("=" * 60)
